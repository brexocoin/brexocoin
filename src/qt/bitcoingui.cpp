...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Brexocoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and brexocoin: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Brexocoin Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Brexocoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Brexocoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a brexocoin: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Brexocoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Brexocoin network", "", count));
    } else {
...
{
    QString strTitle = tr("Brexocoin Core"); // default title
    ...
    // Append title to "Brexocoin Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
