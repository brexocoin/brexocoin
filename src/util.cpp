...
const char * const BITCOIN_CONF_FILENAME = "brexocoin.conf";
const char * const BITCOIN_PID_FILENAME = "brexocoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "brexocoin" is a composite category enabling all Brexocoin-related debug output
                if(ptrCategory->count(std::string("brexocoin"))) {
...
#else
    const char* pszModule = "brexocoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Brexocoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Brexocoin
    // Mac: ~/Library/Application Support/Brexocoin
    // Unix: ~/.brexocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Brexocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Brexocoin";
#else
    // Unix
    return pathRet / ".brexocoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty brexocoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override brexocoin.conf
